...
class WeatherStationDataScreen extends StatefulWidget {
...
class _WeatherStationDataScreenState extends State<WeatherStationDataScreen> {
  ...
  void _processSnapshotData(Map<String, dynamic> data) {
    ...
    final rawStations = _fullApiResponse?['weatherStations'];

    if (rawStations is List) {
      stationsList = rawStations.cast<Map<String, dynamic>>();
    } else if (rawStations is Map<String, dynamic>) {
      stationsList = rawStations.values
          .whereType<List>()
          .expand((list) => list)
  ...
  void _updateAvailableMetricFilters({ bool stationChanged = false }) {
    ...
    for (var key in stationData.keys) {
      if (stationData[key] is List && (stationData[key] as List).isNotEmpty) {
        filters.add(key);
      }
    }
  ...
  void _onStationSelected(String? newStation) {
    if (newStation != null && newStation != _selectedStation) {
      setState(() {
        _selectedStation = newStation;
        _updateAvailableMetricFilters(stationChanged: true);
      });
    }
  }
    ...
    return SizedBox(
      ...
      child: SingleChildScrollView(
        child: Wrap(
          spacing: 8.0,
          runSpacing: 8.0,
          children: _availableStations.map((stationName) {
            final bool isSelected = _selectedStation == stationName;
            final Color color = _getColorForSeries(stationName);
